...
The name-your-own-price mechanism (also known as *reverse pricing* or *name-your-own-price auction*) is an extension of the posted price mechanism. The seller decides the posted price and a threshold level at which he or she is willing to sell the item. *The threshold level bears some similarities with a reserve price in auctions and a price reduction in the posted price mechanism (see footnote 2).* Only the posted price is show to buyers, who now have one additional option besides ‘buy’, and ‘don’t buy’. The buyers can submit a bid. Their bid is rejected if its below the threshold level. And if above, the bid is accepted and the transaction is made with the buyer. This is the essential parts of NYOP, but there exist numerous extensions and modifications. Before is discuss these I would like to breifly present one striking result of the NYOP.

Shapiro and Zillante (2009) finds that profits are higher when using the NYOP mechanism describe above instead of a posted price. One might expect that profits were lower since buyers that could afford the posted price, would instead use the NYOP option, and get the item cheaper. But what drives the result is 1) new buyers that could not afford the item at the posted price, and 2) buyers with values much higher than the posted price, that still use the ‘buy’ option. The latter also puzzles the two authors, since submitting a bid is a dominant strategy in that buyers can choose the to ‘buy’ after their bid has been rejected, in their NYOP setup. The options presented to the buyers after their bid has been rejected in one source for modification of the NYOP. Another modification is to remove the ‘buy’ option from the buyers initial set of choices. Such that the buyer can only ‘bid’ or ‘don’t buy’, if the bid is rejected, then the buyer can ‘buy’ the item at the posted price. Shapiro and Zillante (2009) show that this modification also gives the seller a higher profit than the posted price, but not as big an increase in profit as if the ‘buy’ option had been present. An other modification is the ‘opaque feature’, which hides the posted price from buyers. It will not discuss this in more detail, but just note that Shapiro and Zillante (2009, p.737) hypothesise that fears of buyers learning the threshold level might be the reason why marketplaces have an opaque feature (emphasis added):
...
The NYOP mechanism has mostly been studied as a method to curb excess capacity/supply. On the internet Priceline.com has been the pioneer of this mechanism, selling excess hotel capacity and excess seats on planes (Shapiro and Zillante, 2009). Consequently the literature on NYOP assumes that there are items enough for every buyer willing to purchase. There is no constraint on the number of items for sale and hence no competition among buyers for the items (Shapiro, 2011). In latter sections I will explain how my experiment will try to introduce competition in the NYOP and how it will study learning  through repeated bidding.
